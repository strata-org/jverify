...
public class TestVerifier {
    ...
    private static final Path TEST_FILES_DIRECTORY =
            Path.of("./src/test/java/com/aws/jverify/verifier/tests");

    private static final Path EXAMPLES_DIRECTORY =
    ...
    public void testRunThroughGradle() throws IOException, InterruptedException {
        ...
        var process =
                new ProcessBuilder(
                                gradlePath,
                                ":verifier:run",
                                "--args=\"../examples/src/main/java/com/aws/verifier/examples/Fibonacci.java\"")
                        .start();
        ...
        assertThat(
                output,
                containsString("Dafny program verifier finished with 4 verified, 0 errors"));
    ...
    /**
     * Verifies the given source code and asserts that the exit code, emitted diagnostics, and
     * verified/error counts (from Dafny) match the specified values in the source code's test
     * metadata. See {@link #parseMetadata(String)} for details on the metadata format.
     */
    private static void testMarkedSource(SourceFile markedSourceFile) throws IOException {
        ...
        var diagnosticsAsAnnotations =
                verificationResults.getDiagnostics().stream()
                        .flatMap(
                                diagnostic ->
                                        diagnostic instanceof DafnyDiagnostic dafnyDiagnostic
    ...
    private static final Pattern TEST_METADATA_PATTERN =
            Pattern.compile("^// TEST: (.+)$", Pattern.MULTILINE);

    private record TestMetadata(int exitCode, Integer dafnyVerified, Integer dafnyErrors) {}

    /**
     * Parses and returns test metadata from the given source content, or returns {@code null} if
     * the metadata indicates that the test should be skipped. Throws if the test metadata is absent
     * or malformed.
     *
     * <p>Valid metadata formats:
     *
     * {@snippet :
     * (1)
     * // TEST: skip
     ...
     *
     * <ol>
     *   <li>The test should be skipped.
     *   <li>Verification should finish with exit code {@code X} without Dafny terminating normally
     *       (i.e. Dafny is never invoked because there are javac errors, or Dafny terminates
     *       abnormally).
     *   <li>Verification should finish with exit code {@code X}, Dafny terminates normally, and
     *       Dafny's summary reports {@code Y} verified symbols and {@code Z} errors.
     * </ol>
     */
    private static @Nullable TestMetadata parseMetadata(String source) {
        ...
        assertThat(
                "Metadata must include both or neither of dafnyVerified and dafnyErrors",
    ...
    private static AnnotatedRange diagnosticAsAnnotatedRange(Diagnostic<?> diagnostic) {
        ...
        var endPos =
                diagnostic instanceof DafnyDiagnostic dafnyDiagnostic
                        ? new Position(
                                dafnyDiagnostic.getEndLineNumber(),
                                dafnyDiagnostic.getEndColumnNumber())
    ...
    /**
     * Returns the text with all CRLF sequences replaced with LF. This prevents erroneous failures
     * of diff-based assertions on Windows platforms.
     */
    ...
    private static VerifierOptions getVerifierOptions() {
        ...
        return new VerifierOptions(
                ...
                null,
                true,
                new String[] {"--use-basename-for-filename"
                    // ,"--wait-for-debugger"
                });
    }
}
