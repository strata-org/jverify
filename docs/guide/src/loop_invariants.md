...
In the next sections, [Pre- and post-conditions](pre_and_postconditions.md), [Code types](code_types.md), [Forall and exists](forall_and_exists.md), we will introduce the tools needed to prove the correctness of the above `binarySearch` example, and we will also use invariants to prove facts that hold after the loop exits.