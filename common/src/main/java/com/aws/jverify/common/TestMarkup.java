// Modified after being taken from
// https://github.com/dotnet/roslyn/blob/576abfa9a8f2d64967ceabe7fa0e5b7ebe550752/src/Compilers/Test/Core/MarkedSource/MarkupTestFile.cs
...
/**
 * To aid with testing, we define a special type of text file that can encode additional information
 * in it. This prevents a test writer from having to carry around multiple sources of information
 * that must be reconstituted. For example, instead of having to keep around the contents of a file
 * *and* and the location of the cursor, the tester can just provide a string with the "><" string
 * in it. This allows for easy creation of "FIT" tests where all that needs to be provided are
 * strings that encode every bit of state necessary in the string itself.
 *
 * <p>The current set of encoded features we support are:
 *
 * <p>>< - The position in the file. There can be zero or more of these.
 *
 * <p>[> ... <] - A span of text in the file. There can be many of these, and they can be nested
 * and/or overlap the >< position(s).
 *
 * <p>{>Name: ... <} A span of text in the file annotated with an identifier. There can be many of
 * these, including ones with the same name.
 *
 * <p>(>metadata::: ... <) A span of text in the file annotated with metadata.
 *
 * <p>We also support Java line comments to specify annotated ranges:
 *
 * {@snippet :
...
public class TestMarkup {
    ...
    private static String parse(String input, List<Integer> positions, List<AnnotatedSpan> spans) {
        ...
        while (matcher.find()) {
            ...
            if (matcher.group("Position") != null) {
                ...
                annotatedSpanStartStack.push(
                        new Span(matchIndexInOutput, matcher.group("Annotation")));
            } else if (matcher.group("StandaloneAnnotation") != null) {
                annotatedSpanStartStack.push(
                        new Span(matchIndexInOutput, matcher.group("StandaloneAnnotation")));
        ...
        if (!spanStartStack.isEmpty()) {
            throw new IllegalArgumentException(
                    "Saw " + SPAN_START_STRING + " without matching " + SPAN_END_STRING);
        }
    ...
    private static void popSpan(
            Stack<Span> spanStartStack, List<AnnotatedSpan> spans, int finalIndex) {
    ...
    public record GetPositionsAndNamedRangesResult(
            String output, List<Position> positions, Map<String, List<Range>> namedRanges) {}
    ...
    public static GetPositionsAndNamedRangesResult getPositionsAndNamedRanges(String input) {
        ...
        for (AnnotatedRange annotatedRange : output.ranges) {
            namedRanges
                    .computeIfAbsent(annotatedRange.annotation(), _ -> new ArrayList<>())
    ...
    public record GetPositionsAndAnnotatedRangesResult(
            String output, List<Position> positions, List<AnnotatedRange> ranges) {}

    public static GetPositionsAndAnnotatedRangesResult getPositionsAndAnnotatedRanges(
            String input) {
    ...
    public static String getPositionsAndRanges(
            String input, List<Position> positions, List<Range> ranges) {
        List<Integer> positionIndices = new ArrayList<>();
        List<AnnotatedSpan> spans = new ArrayList<>();
        var output = getIndexAndSpans(input, positionIndices, spans);
    ...
    public static GetPositionAndRangesResult getPositionAndRanges(
            String input, List<Range> ranges) {
        List<Position> positions = new ArrayList<>();
